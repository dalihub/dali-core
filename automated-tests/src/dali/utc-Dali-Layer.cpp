...
int UtcDaliLayerIgnored(void)
{
  ...
  // For line coverage.
  rootLayer.SetIgnored(true);
  parent.SetProperty(Actor::Property::POSITION, Vector2(100, 300));
  ...
  childPosition2 = child2.GetProperty<Vector3>(Actor::Property::WORLD_POSITION);
  DALI_TEST_EQUALS(childPosition1, childPosition2, TEST_LOCATION);
  DALI_TEST_EQUALS(childPosition1, Vector3(100, 300, 0), TEST_LOCATION);
...
int utcDaliLayerPartialUpdate(void)
{
  ...
  // Aligned by 16
  clippingRect  = BoundsInteger(0, 752, 80, 48);  // in screen coordinates
  expectedRect1 = BoundsInteger(0, 752, 48, 48);  // in screen coordinates, includes 1 last frames updates
  expectedRect2 = BoundsInteger(16, 752, 48, 48); // in screen coordinates, includes 1 last frames updates
