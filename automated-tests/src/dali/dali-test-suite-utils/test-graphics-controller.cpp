...
Graphics::UniquePtr<Graphics::Program> TestGraphicsController::CreateProgram(const Graphics::ProgramCreateInfo& programCreateInfo, Graphics::UniquePtr<Graphics::Program>&& oldProgram)
{
  ...
  mProgramCache.back().programImpl = std::make_shared<TestGraphicsProgramImpl>(*this, mGl, programCreateInfo, mVertexFormats, mCustomUniforms, mCustomUniformBlocks);
...
const Matrix& TestGraphicsController::GetClipMatrix(const Graphics::RenderTarget* renderTarget) const
{
  ...
  // For Vulkan, If render target is not surface, invert Y scale.
