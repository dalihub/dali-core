...
struct UniformData
{
  std::string    name;
  Property::Type type;
  explicit UniformData(const std::string& name, Property::Type type = Property::Type::NONE)
  : name(name),
    type(type)
  {
  }
...
{
public:
  TestGraphicsProgramImpl(TestGraphicsController& controller, const Graphics::ProgramCreateInfo& createInfo, Property::Array& vertexFormats, std::vector<UniformData>& customUniforms, std::vector<TestGraphicsReflection::TestUniformBlockInfo>& customUniformBlocks);
