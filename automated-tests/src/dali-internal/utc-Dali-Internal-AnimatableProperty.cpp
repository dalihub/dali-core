...
int UtcDaliInternalPropertyGetValueAddress(void)
{
  ...
  static_cast<GesturePropertyVector2*>(properties.back().property.get())->Set(properties.back().value.Get<Vector2>());
  ...
  for(auto& valueSizeTestCase : properties)
  {
    ...
    switch(valueSizeTestCase.value.GetType())
    {
      ...
      {
        DALI_TEST_EQUALS(valueSizeTestCase.value.Get<bool>(), *reinterpret_cast<bool*>(addr), TEST_LOCATION);
        break;
      }
      case Property::Type::INTEGER:
      {
        DALI_TEST_EQUALS(valueSizeTestCase.value.Get<int>(), *reinterpret_cast<int*>(addr), TEST_LOCATION);
        break;
      }
      case Property::Type::FLOAT:
      {
        DALI_TEST_EQUALS(valueSizeTestCase.value.Get<float>(), *reinterpret_cast<float*>(addr), TEST_LOCATION);
        break;
      }
      case Property::Type::VECTOR2:
      {
        DALI_TEST_EQUALS(valueSizeTestCase.value.Get<Vector2>(), *reinterpret_cast<Vector2*>(addr), TEST_LOCATION);
        break;
      }
      case Property::Type::VECTOR3:
      {
        DALI_TEST_EQUALS(valueSizeTestCase.value.Get<Vector3>(), *reinterpret_cast<Vector3*>(addr), TEST_LOCATION);
        break;
      }
      case Property::Type::VECTOR4:
      {
        DALI_TEST_EQUALS(valueSizeTestCase.value.Get<Vector4>(), *reinterpret_cast<Vector4*>(addr), TEST_LOCATION);
        break;
      }
      case Property::Type::MATRIX:
      {
        DALI_TEST_EQUALS(valueSizeTestCase.value.Get<Matrix>(), *reinterpret_cast<Matrix*>(addr), TEST_LOCATION);
        break;
      }
      case Property::Type::MATRIX3:
      {
        DALI_TEST_EQUALS(valueSizeTestCase.value.Get<Matrix3>(), *reinterpret_cast<Matrix3*>(addr), TEST_LOCATION);
        break;
      }
      default:
        break;
