...
bool RayTest::SphereTest(const Internal::Actor& actor, const Vector4& rayOrigin, const Vector4& rayDir)
{
  ...
  const float width  = size.width * scale.width + touchHitAreaMargin.start + touchHitAreaMargin.end;
  const float height = size.height * scale.height + touchHitAreaMargin.top + touchHitAreaMargin.bottom;
  ...
  float a  = rayDir.LengthSquared3();                                 // a
...
bool RayTest::ActorTest(const Internal::Actor& actor, const Vector4& rayOrigin, const Vector4& rayDir, Vector2& hitPointLocal, float& distance)
{
  ...
  if(actor.OnScene())
  {
    ...
    if(fabsf(b) > Math::MACHINE_EPSILON_1)
    {
      ...
      hit = (hitPointLocal.x >= -touchHitAreaMargin.start - epsilon) && (hitPointLocal.x <= (size.x + touchHitAreaMargin.end + epsilon) && (hitPointLocal.y >= -touchHitAreaMargin.top - epsilon) && (hitPointLocal.y <= (size.y + touchHitAreaMargin.bottom + epsilon)));
    }
  }
