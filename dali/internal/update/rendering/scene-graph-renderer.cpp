...
void Renderer::PrepareRender( Graphics::API::Controller& controller, BufferIndex updateBufferIndex )
{
  ...
  for( auto i = 0u; i < uboCount; ++i )
  {
    ...
    if( mUboMemory.size() <= i )
    {
      mUboMemory.resize( i + 1);
    }
    ...
    for( auto&& j : i )
    {
      ...
      if( shader.GetNamedUniform( j->uniformName, uniformInfo ) )
      {
        ...
        switch( j->propertyPtr->GetType() )
        {
          ...
          {
            std::cout << uniformInfo.name << ":["<<uniformInfo.bufferIndex<<"]: " << "Writing 32bit offset: " << uniformInfo.offset << std::endl;
            memcpy( dst, &j->propertyPtr->GetFloat( updateBufferIndex ), sizeof(float) );
            break;
          }
          case Property::Type::VECTOR2:
          {
            std::cout << uniformInfo.name << ":["<<uniformInfo.bufferIndex<<"]: " << "Writing vec2 offset: " << uniformInfo.offset << std::endl;
            memcpy( dst, &j->propertyPtr->GetVector2( updateBufferIndex ), sizeof(Vector2) );
            break;
          }
          case Property::Type::VECTOR3:
          {
            std::cout << uniformInfo.name << ":["<<uniformInfo.bufferIndex<<"]: " <<  "Writing vec3 offset: " << uniformInfo.offset << std::endl;
            memcpy( dst, &j->propertyPtr->GetVector3( updateBufferIndex ), sizeof(Vector3) );
            break;
          }
          case Property::Type::VECTOR4:
          {
            std::cout << uniformInfo.name << ":["<<uniformInfo.bufferIndex<<"]: " << "Writing vec4 offset: " << uniformInfo.offset << std::endl;
            memcpy( dst, &j->propertyPtr->GetVector4( updateBufferIndex ), sizeof(Vector4) );
            break;
          }
          case Property::Type::MATRIX:
          {
            std::cout << uniformInfo.name << ":["<<uniformInfo.bufferIndex<<"]: " << "Writing mat4 offset: " << uniformInfo.offset << std::endl;
            memcpy( dst, &j->propertyPtr->GetMatrix( updateBufferIndex ), sizeof(Matrix) );
            break;
          }
          case Property::Type::MATRIX3:
          {
            std::cout << uniformInfo.name << ":["<<uniformInfo.bufferIndex<<"]: " << "Writing mat3 offset: " << uniformInfo.offset << std::endl;
  ...
  mGfxRenderCommand.Draw( std::move(Graphics::API::RenderCommand::DrawCommand{}
                   ...
                   .SetFirstInstance(0u)
                   .SetVertexCount( mGeometry->GetVertexBuffers()[0]->GetElementCount() )
                   .SetInstanceCount( 1u )));
