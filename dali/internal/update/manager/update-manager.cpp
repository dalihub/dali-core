...
void UpdateManager::UpdateNodes( BufferIndex bufferIndex )
{
  ...
  for( auto&& rootLayer : mImpl->roots )
  {
    ...
    mImpl->nodeDirtyFlags |= UpdateNodeTree( *rootLayer, bufferIndex );
  }
}
...
{
  auto mainRenderTask = mImpl->taskLists[0]->GetTasks()[0];
  mainRenderTask->SetClearEnabled( true );
  mainRenderTask->SetClearColor( 0, color );
  mainRenderTask->SetClearColor( 1, color );
}
