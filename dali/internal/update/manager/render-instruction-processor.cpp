  ...
  if(requiredInsideCheck())
  {
    ...
    if(inside && !isLayer3d && viewportSet)
    {
      ...
      if(sizeXY.LengthSquared() > Math::MACHINE_EPSILON_1000)
      {
        ...
        Dali::BoundsInteger boundingBox = RenderItem::CalculateTransformSpaceAABB(nodeModelViewMatrix, Vector3(nodeUpdateArea.x, nodeUpdateArea.y, 0.0f), Vector3(nodeUpdateArea.z, nodeUpdateArea.w, 0.0f));
        Dali::BoundsInteger clippingBox = camera.GetOrthographicClippingBox();
