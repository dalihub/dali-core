...
public:
   ...
   * @param flags flags
   */
  virtual void CopyBuffer(const API::Buffer& buffer,
  ...
  virtual bool TryConvertPixelData( const void* pData, uint32_t sizeInBytes, uint32_t width, uint32_t height, std::vector<uint8_t>& outputBuffer ) = 0;

  virtual bool TryConvertPixelData( const void* pData, uint32_t sizeInBytes, uint32_t width, uint32_t height, void* outputBuffer ) = 0;
