...
#pragma GCC diagnostic pop
  ...
  void SubmitCommandV2( Dali::Graphics::API::RenderCommand&& command )
  {
    ...
    // TODO: defaults, should be provided by the buffer info
    const auto VERTEX_INPUT_ATTRIBUTE_DESCRIPTION_DEFAULT = std::vector<vk::VertexInputAttributeDescription>{
    ...
    const auto VERTEX_INPUT_BINDING_DESCRIPTION_DEFAULT = std::vector<vk::VertexInputBindingDescription>{
    ...
    // prepare state info
    auto stateInfo = GraphicsPipelineState::Info{};
    stateInfo.vertexShader = vsh;
    stateInfo.fragmentShader = fsh;
    stateInfo.blendEnabled = VK_TRUE;
    stateInfo.depthTestEnabled = VK_TRUE;
    stateInfo.stencilTestEnabled = VK_FALSE;
    stateInfo.topology = vk::PrimitiveTopology::eTriangleList;
    stateInfo.vertexAttributeDescription = VERTEX_INPUT_ATTRIBUTE_DESCRIPTION_DEFAULT;
    stateInfo.vertexInputBindingDescripton = VERTEX_INPUT_BINDING_DESCRIPTION_DEFAULT;
    stateInfo.viewport = vk::Rect2D{}; // fullscreen

    // check if we have state, binary compare should be enough
    GraphicsPipelineState* pPipeline = nullptr;
    for( auto&& state : mStatePerBatchCache )
    {
      if( state == stateInfo )
      {
        pPipeline = &state;
    ...
    // create new pipeline based on submitted state
    if( !pPipeline )
    {
      GraphicsPipelineState tempState( stateInfo  );
      if( tempState.Initialise( mGraphics ) )
      {
        mStatePerBatchCache.push_back( std::move(tempState) );
      }
      pPipeline = &mStatePerBatchCache.back();
    }
    ...
    for(const auto& bufferInfo : bufferVector )
    {
      ...
      SubmitDraw( *pPipeline, bufferInfo, idx++ );
    }
  }
