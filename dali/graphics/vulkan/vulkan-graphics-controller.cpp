...
#pragma GCC diagnostic pop
  ...
  void SubmitCommands( std::vector<Dali::Graphics::API::RenderCommand*> commands )
  {
    ...
    {
      auto apiCommand = static_cast<VulkanAPI::RenderCommand*>(command);
      const auto& pushConstants = apiCommand->GetPushConstantsBindings();
      for( const auto& pc : pushConstants )
      {

        /**
         * does render command has Ubo assigned yet? should happen when
         * assigning pipeline
         * if( renderCommand.ubo.empty() )
         *  assert()
         *
         */
        ...
        if(!ubo.mapped)
        {
          ubo.Map(); // maps whole ubo if not mapped yet
        }


        void* ptr = ubo.GetPtr(index);
        std::copy();
    ...
    for( auto&& command : commands )
    {
      ...
      // figure out where to bind textures
      const auto& textureBindings = apiCommand->GetTextureBindings();
      ...
      for( const auto& pc : pushConstants )
      {
        std::cout << pc.binding << std::endl;



      }

      // either pool for each render item ( 1 pool/ 1 descriptor set )
      // or something else
      Vulkan::DescriptorPoolRef poolRef = Vulkan::DescriptorPool::New( mGraphics, vk::DescriptorPoolCreateInfo{}
      .setMaxSets(1)
      .setPoolSizeCount(1)
      .setPPoolSizes( nullptr ));

      //Vulkan::DescriptorSetRef ref = poolRef->AllocateDescriptorSets(  )
  ...
  void BindPipeline( const Dali::Graphics::API::RenderCommand& renderCommand )
  {
    ...
    */
    ...
    // make a copy of vertex buffer and attribute bindings
    info.vertexBufferAttributesBindings = Dali::Graphics::API::RenderCommand::NewVertexAttributeBufferBindings( renderCommand.GetVertexBufferBindings() );

    // build pipeline based on state
