...
#pragma GCC diagnostic pop
  ...
  void* CreateTexture( void* data, size_t sizeInBytes, uint32_t width, uint32_t height )
  {
    ...
    {
      auto inData = reinterpret_cast<uint8_t*>(data);
  ...
  const API::ShaderFactory& GetShaderFactory() const
  {
  ...
  void SubmitCommandV2( Dali::Graphics::API::RenderCommand&& command )
  {
    ...
    if(vertexShader)
    {
      vsh = VulkanAPI::VulkanShader::DownCast( vertexShader.Get() ).GetShader( vk::ShaderStageFlagBits::eVertex );
    }

    if(fragmentShader)
    {
      fsh = VulkanAPI::VulkanShader::DownCast( vertexShader.Get() ).GetShader( vk::ShaderStageFlagBits::eFragment );
    }
...
{
  mImpl->SubmitCommandV2( std::move( command ) );
...
const API::ShaderFactory& Controller::GetShaderFactory() const
{
