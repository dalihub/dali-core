    ...
    while( totalSetsToAllocate > 0 )
    {
      ...
      for( auto& pool : pools )
      {
        ...
        if( pool->GetAvailableAllocations() > 0 )
        {
          ...
          //TODO: This is a bug. Branch 1 logic is flawed
      ...
      if( totalSetsToAllocate > 0 )
      {
        ...
        auto division = static_cast< double >( totalSetsToAllocate ) / static_cast< double >( mMaxPoolSize );
...
void DescriptorSetAllocator::PrintAllocationReport()
{
