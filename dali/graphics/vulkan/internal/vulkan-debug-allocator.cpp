...
struct AllocationTracker
{
  ...
  void* valloc( size_t size, size_t align, VkSystemAllocationScope scope, const char* debugString )
  {
    ...
    if( debugString )
    {
      allocation.debuString = debugString;
    }
    ...
    allocationData[retval] = allocation;
    return retval;
  }

  void* vrealloc( void* original, size_t size, size_t align, VkSystemAllocationScope scope, const char* debugString  )
  {
    ...
    for( auto& item : allocationData )
    {
      if( !item.second.freed )
      {
        if( item.second.allocationScope == VK_SYSTEM_ALLOCATION_SCOPE_OBJECT )
        {
          ...
          if(item.second.debuString == "CreateImageView()")
            imageViewCount++;
          else if(item.second.debuString == "CreateImage()")
            imageCount++;
          else if(item.second.debuString == "CreateSampler()")
            samplerCount++;
          else if(item.second.debuString == "CreateBuffer()")
            bufferCount++;
          else if(item.second.debuString == "DeviceMemory")
            deviceMemoryCount++;
          else if(item.second.debuString == "ShaderModule")
            shaderModuleCount++;
          else if(item.second.debuString == "DescriptorPool")
          {
            dsPoolCount++;
            dsPoolMemory += item.second.size;
          }
          else if(item.second.debuString == "Pipeline")
            pipelineCount++;
          else if(item.second.debuString == "PipelineLayout")
            pipelineLayoutCount++;
          else if(item.second.debuString == "DescriptorSetLayout")
            dsLayoutCount++;
        }
  ...
  VkSystemAllocationScope                     allocationScope)
{
  auto retval = gAllocationTracker.valloc( size, alignment, allocationScope,
                                           pUserData ? reinterpret_cast<const char*>(pUserData) : nullptr );
  if(pUserData)
  {
    //free(pUserData);
    pUserData = nullptr;
