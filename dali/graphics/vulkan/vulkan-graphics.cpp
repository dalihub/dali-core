...
VkBool32 VulkanReportCallback(
  ...
  void*                                       pUserData)
{
  DALIEXT_ERRLOG( "ADAM: %s", pMessage );
  return VK_FALSE;
}
#endif
                               ...
                               const std::vector< const char* >& validationLayers )
{
  ...
  PFN_vkCreateDebugReportCallbackEXT func = reinterpret_cast<PFN_vkCreateDebugReportCallbackEXT>(mInstance.getProcAddr( "vkCreateDebugReportCallbackEXT" ));
  func( static_cast<VkInstance>(mInstance), &reportInfo, nullptr, &vkCallback );
#endif
}
