...
#define CLASS "DescriptorSetAllocator::"
namespace Dali
{
namespace Graphics
{
...
{
  printf(CLASS "UpdateWithRequirements: bufferIndex = %d\n", int(mBufferIndex));
  ...
  for( auto& requirement : requirements )
  {
    ...
    if( it == poolset.end() )
    {
      printf( CLASS "Added pool for new signature\n");
    ...
    printf(CLASS "Unallocated sets: %d\n", int(requirement.unallocatedSets));
    if( requirement.unallocatedSets >= pool.available )
    {
      pool.requiredSets *= 2; // double
      pool.shouldReallocate = true;
      printf(CLASS "Pool will resize, required sets = %d\n", int(pool.requiredSets));
    }
    else
    {
      pool.shouldReallocate = false;
  ...
  for( auto& pool : poolset )
  {
    if( pool.shouldReallocate )
    {
      ...
      auto newPool = graphics.GetDevice().createDescriptorPool( createInfo, graphics.GetAllocator() ).value;
      ...
      if( oldPool )
      {
        ...
        graphics.DiscardResource( [device, allocator, oldPool](){
          // todo: invalidate old descriptor sets

          device.destroyDescriptorPool( oldPool, allocator );
        } );
      }
...
{
  printf(CLASS "Allocating descriptor sets bufferIndex = %d\n", int(mBufferIndex));
  auto& poolset = mPoolSet[mBufferIndex];
  ...
  for( auto& signature : signatures )
  {
    ...
    if( !(*it).dirty )
    {
      puts(CLASS "No need to allocate, nothing changed");
      continue;
    }
...
void DescriptorSetAllocator::FreeDescriptorSets( const std::vector<DescriptorSetList>& descriptorSets )
{
  // build free lists
