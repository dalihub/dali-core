  ...
  for( auto& pool : mPoolSet[mBufferIndex] )
  {
    if( !pool.vkDescriptorSetsToBeFreed.empty() )
    {
      ...
      std::for_each( pool.vkDescriptorSets.begin(), pool.vkDescriptorSets.end(), [&]( auto& item )
      {
        ...
        {
          printf("F:%u, freed descriptor set= %lu\n", mController.GetFrameCount(),
          uintptr_t(static_cast<VkDescriptorSet>(item)));
        }
      });
  ...
  DescriptorSetList& descriptorSets )
{
  ...
  for( auto& signature : signatures )
  {
    ...
    if( it == poolset.end() )
    {
      bb.log() << "AllocateDescriptorSets:: pool set not found" << bb.end();
      continue;
    }

    if( !(*it).dirty && !descriptorSets.descriptorSets.empty() )
    {
      bb.log() << "Neither dirty not empty...dirty = "
               << (*it).dirty
               << ", empty == " << descriptorSets.descriptorSets.empty()
               << bb.end();
