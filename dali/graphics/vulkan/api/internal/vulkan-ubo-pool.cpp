...
struct UboPool::Impl
{
  ...
  void NewUboBuffer()
  {
    ...
    mBuffers.back()->BindMemory(
              graphics.GetDeviceMemoryManager().GetDefaultAllocator().Allocate( mBuffers.back(), vk::MemoryPropertyFlagBits::eHostVisible )
    );
  ...
  void* Map( Ubo& ubo )
  {
    ...
    return (mBuffers[bufferIndex]->GetMemoryHandle()->MapTyped<char>() + (allocationIndex*mBlockSize));
  }
  ...
  void Unmap( Ubo& ubo )
  {
    auto& impl = ubo.GetImplementation();
    auto bufferIndex = uint32_t(impl.mAllocationIndex / mInitialCapacity);
    mBuffers[bufferIndex]->GetMemoryHandle()->Unmap();
  }
