...
bool Shader::GetUniformBlock( uint32_t index, API::ShaderDetails::UniformBlockInfo& out ) const
{
  ...
  info.binding = block.binding;
  info.descriptorSet = block.descriptorSet;
  info.members.resize( block.members.size() );
  info.size = block.size;
  for( auto i = 0u; i < info.members.size(); ++i )
  {
    const auto& memberUniform = block.members[i];
    info.members[i].name = memberUniform.name;
    info.members[i].binding = block.binding;
    info.members[i].uniformClass = API::ShaderDetails::UniformClass::UNIFORM_BUFFER;
    info.members[i].offset = memberUniform.offset;
    info.members[i].location = memberUniform.location;
  }
