...
uint32_t HashPipeline(const VulkanAPI::PipelineFactory::Info &info)
{
  ...
  uint32_t viStateBindingsHash = HashBinary( info.vertexInputState.bufferBindings.data(), uint32_t(sizeof( Pipeline::VertexInputState::Binding )*info.vertexInputState.bufferBindings.size()) );
  uint32_t viStateAttributesHash = HashBinary( info.vertexInputState.attributes.data(), uint32_t(sizeof( Pipeline::VertexInputState::Attribute )*info.vertexInputState.attributes.size()) );
...
API::PipelineFactory& PipelineFactory::SetPipelineCache(API::PipelineCache& pipelineCache)
{
...
API::PipelineFactory& PipelineFactory::SetColorBlendState(const API::Pipeline::ColorBlendState& state )
{
...
API::PipelineFactory& PipelineFactory::SetShaderState( const API::Pipeline::ShaderState& state )
{
  mInfo.shaderState = state;
  return *this;
}

API::PipelineFactory& PipelineFactory::SetViewportState( const API::Pipeline::ViewportState& state )
{
...
API::PipelineFactory& PipelineFactory::SetDepthStencilState( API::Pipeline::DepthStencilState state )
{
  mInfo.depthStencilState = state;
  return *this;
}

API::PipelineFactory& PipelineFactory::SetRasterizationState( const API::Pipeline::RasterizationState& state )
{
  mInfo.rasterizationState = state;
  return *this;
}

API::PipelineFactory& PipelineFactory::SetVertexInputState( const API::Pipeline::VertexInputState& state )
{
  mInfo.vertexInputState = state;
  return *this;
}

API::PipelineFactory& PipelineFactory::SetInputAssemblyState( const API::Pipeline::InputAssemblyState& state )
{
...
{
  return HashPipeline( GetCreateInfo() );
}
