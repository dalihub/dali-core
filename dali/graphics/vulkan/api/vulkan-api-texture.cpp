...
struct Texture::Impl
{
  ...
  bool Initialise()
  {
    ...
    if( data )
    {
      CopyMemory(data, {mWidth, mHeight}, {0, 0}, 0, 0);
    }
    return true;
  }

  void CopyMemory(const void *srcMemory, API::Extent2D srcExtent, API::Offset2D dstOffset, uint32_t layer, uint32_t level)
  {
  ...
  void CopyTexture(const API::Texture &srcTexture, API::Rect2D srcRegion, API::Offset2D dstOffset, uint32_t layer, uint32_t level)
  {
  ...
  void CopyBuffer(const API::Buffer &srcBuffer, API::Extent2D srcExtent, API::Offset2D dstOffset, uint32_t layer, uint32_t level)
  {
    ResourceTransferRequest transferRequest( TransferRequestType::BUFFER_TO_IMAGE );
...
{
  mImpl->CopyMemory(srcMemory, srcExtent, dstOffset, layer, level);
}
...
{
  mImpl->CopyTexture(srcTexture, srcRegion, dstOffset, layer, level);
}
...
{
  mImpl->CopyBuffer(srcBuffer, srcExtent, dstOffset, layer, level);
}
