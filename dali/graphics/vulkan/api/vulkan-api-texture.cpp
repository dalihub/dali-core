...
Texture::Texture( Dali::Graphics::API::TextureFactory& factory )
  : mTextureFactory( dynamic_cast<VulkanAPI::TextureFactory&>( factory ) ),
    ...
    mComponentMapping()
{
  DALI_LOG_ERROR("%s: Texture Construct---------------------------------->\n",__FUNCTION__);
}

Texture::~Texture()
{
  DALI_LOG_ERROR("%s: Texture Destruct<----------------------------------\n",__FUNCTION__);
  if ( mYcbcrConv )
  {
...
bool Texture::Initialise()
{
  ...
  {
    DALI_LOG_ERROR("%s:Native Image Case\n",__FUNCTION__);
  ...
  {
    DALI_LOG_ERROR("%s:Normal Image Case\n",__FUNCTION__);
...
//  DALI_LOG_ERROR("%s:<<------------------------------------------>>\n",__FUNCTION__);
  ...
  {
    DALI_LOG_ERROR("%s:tbmSurface's format is TBM_FORMAT_NV21\n",__FUNCTION__);
  ...
  {
    DALI_LOG_ERROR("%s:tbmSurface's format is ARGB\n",__FUNCTION__);
      ...
      || tbmSurface_info.format == TBM_FORMAT_BGRA8888)
  {
    ...
    DALI_LOG_ERROR("%s:tbmSurface's format is BGRA\n",__FUNCTION__);
    //mFormat = vk::Format::eB8G8R8A8Unorm;
    mFormat = vk::Format::eR8G8B8A8Unorm;
  }
