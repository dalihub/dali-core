...
struct ColorConversion
{
  ...
  void(*pConversionWriteFunc)(const void*, uint32_t, uint32_t, uint32_t, uint32_t, void*, uint32_t);
};
...
inline void WriteRGB32ToRGBA32( const void* pData, uint32_t sizeInBytes, uint32_t width, uint32_t height, uint32_t rowStride, void* pOutput, uint32_t outputBufferSize )
{
  auto inData = reinterpret_cast<const uint8_t*>(pData);
  ...
  for( auto i = 0u; i < sizeInBytes; i += 3 )
  {
    if( outIdx >= outputBufferSize ) // overflow
    {
      return;
    }
    outData[outIdx] = inData[i];
...
bool Texture::TryConvertPixelData( const void* pData, uint32_t sizeInBytes, uint32_t width, uint32_t height, void* pOutputBuffer, uint32_t outputBufferSize )
{
  // No need to convert
  if( mConvertFromFormat == vk::Format::eUndefined )
  {
  ...
  it->pConversionWriteFunc( begin, sizeInBytes, width, height, 0u, pOutputBuffer, outputBufferSize );
