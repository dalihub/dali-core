...
: mController( controller ), mGraphics( graphics ), mPipelineCache( pipelineCache ), mCommandBuffer(), mPipeline(), mUpdateFlags( 0u )
{
}
...
{
  auto& uboManager = mController.GetUboManager();
  {
    auto ubo0 = uboManager.Allocate(288);

    auto somedata = new char[512];
    ubo0->WriteKeepMapped( somedata, 0, 288 );
    delete [] somedata;

    auto ubo1 = uboManager.Allocate(340);
    auto ubo2 = uboManager.Allocate(12);
    auto ubo3 = uboManager.Allocate(32);
  }
  ...
  if(!pipelineRef)
  {
    ...
    auto pipelineLayout = VkAssert(mGraphics.GetDevice()
                                            .createPipelineLayout(pipelineLayoutCreateInfo, mGraphics.GetAllocator()));
    ...
    pipelineCache.AddPipeline(pipeline, Vulkan::PipelineDescription{vertexShader, fragmentShader});
    ...
    mDescriptorPool = Vulkan::DescriptorPool::New( mGraphics, vk::DescriptorPoolCreateInfo{}
                .setMaxSets( Vulkan::U32(dsLayouts.size()+1) )
