...
bool RenderCommand::PreparePipeline()
{
  ...
  mGraphics.GetDevice().destroyPipelineLayout( pipelineLayout, mGraphics.GetAllocator() );
  for( auto& dsLayout : dsLayouts )
  {
    mGraphics.GetDevice().destroyDescriptorSetLayout( dsLayout, mGraphics.GetAllocator() );
  }
