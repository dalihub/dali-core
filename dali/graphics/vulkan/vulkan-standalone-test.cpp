...
using UGraphics = std::unique_ptr< Graphics >;

class VkSurfaceXlib : public VkSurfaceFactory
{
public:
  /**
                                ...
                                vk::PhysicalDevice physicalDevice) const override
  {
    ...
    auto retval = VkAssert(instance.createXlibSurfaceKHR(info, allocCallbacks));
    return retval;
  }
...
class VkSurfaceXcb : public VkSurfaceFactory
{
public:
  /**
   * Instantiates surface factory ( should
                                ...
                                vk::PhysicalDevice physicalDevice) const override
  {
    ...
    auto retval = VkAssert(instance.createXcbSurfaceKHR(info, allocCallbacks));
    return retval;
  }
...
#endif
  UGraphics graphics{new Graphics()};
  graphics->Create();

  // store framebuffer id
  FBID fbid = graphics->CreateSurface(std::move(surfaceFactory));

  // initialise logical device
  graphics->CreateDevice();

  // run render loop for particular surface
  auto &surface = graphics->GetSurface(fbid);
