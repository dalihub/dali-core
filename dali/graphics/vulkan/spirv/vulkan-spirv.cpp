...
struct SPIRVShader::Impl
{
  ...
  bool Initialise()
  {
    ...
    for( auto&& name : names )
    {
      ...
      if( !strName.empty() && !name.isMemberName )
      {
        ...
        uint32_t resourceBlockBuffer{0u};
  ...
  void GenerateDescriptorSetLayoutCreateInfo( const std::vector<ResourceDescriptor*>& uniformResources )
  {
    ...
    for( auto&& resource : uniformResources )
    {
      ...
      if( resource->descriptorSet != currentSet )
      {
        ...
        if( layoutCreateInfoCache.size() <= currentSet )
        {
  ...
  std::vector<vk::DescriptorSetLayoutCreateInfo> GenerateDescriptorSetLayoutCreateInfo()
  {
...
std::vector<vk::DescriptorSetLayoutCreateInfo> SPIRVShader::GenerateDescriptorSetLayoutCreateInfo()
{
