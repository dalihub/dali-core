...
void SwapchainImpl::BeginFrame()
{
  ...
  cmdbuf->pipelineBarrier(vk::PipelineStageFlagBits::eTopOfPipe, vk::PipelineStageFlagBits::eTopOfPipe,
                          vk::DependencyFlags(), 0, nullptr, 0, nullptr, static_cast<uint32_t>(barrierIndex), &barriers[0]);
...
void SwapchainImpl::BeginRenderPass( CommandBuffer& cmdbuf )
{
  ...
  area.setOffset({0, 0})
      .setExtent({mSurface.GetWidth(), mSurface.GetHeight()});
  ...
  // clear color
  clearValues[0] = vk::ClearValue().color
                                   .setFloat32({red, 0.0f, 0.0f, 1.0f});

  // clear depth/stencil
  clearValues[1] = vk::ClearValue().depthStencil
                    .setDepth(1.0f)
                    .setStencil(0);
